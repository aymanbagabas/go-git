...
func NegotiatePack(
	...
	if req.Filter != "" {
		if caps.Supports(capability.Filter) {
			...
			if err := upreq.Capabilities.Set(capability.Filter); err != nil {
				return nil, fmt.Errorf("setting filter capability: %w", err)
			}
		} else {
	...
	if req.Depth > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("getting shallow commits: %w", err)
		}
	}
	...
	if isSubset(req.Wants, req.Haves) && len(upreq.Shallows) == 0 {
		if err := pktline.WriteFlush(writer); err != nil {
			return nil, fmt.Errorf("writing flush packet: %s", err)
		}
