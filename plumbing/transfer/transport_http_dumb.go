...
func (c *HTTPConn) fetchDumb(ctx context.Context, st storage.Storer, req *FetchRequest) error {
	...
	r := newFetchWalker(c, ctx, repoFs)
	if err := r.process(st, c.refs); err != nil {
		return err
	}

	if err := r.fetch(st); err != nil {
...
func (r *fetchWalker) getInfoPacks() ([]string, error) {
	url, err := url.JoinPath(r.url.String(), "objects", "info", "packs")
	if err != nil {
		return nil, err
	}
...
func (r *fetchWalker) downloadFile(fp string) (rErr error) {
	url, err := url.JoinPath(r.url.String(), fp)
	if err != nil {
		return err
	}
	req, err := http.NewRequestWithContext(r.ctx, http.MethodGet, url, nil)
...
func (r *fetchWalker) getHead() (ref *plumbing.Reference, err error) {
	url, err := url.JoinPath(r.url.String(), "HEAD")
	if err != nil {
		return nil, err
	}
	req, err := http.NewRequestWithContext(r.ctx, http.MethodGet, url, nil)
...
func (r *fetchWalker) process(st storage.Storer, refs *packp.AdvRefs) error {
	...
	if refs.Head == nil {
		...
		case plumbing.SymbolicReference:
			for name, h := range refs.References {
	...
	for name, hash := range refs.References {
		peeled, hasPeeled := refs.Peeled[name]
		if st.HasEncodedObject(hash) != nil {
			r.queue = append(r.queue, hash)
		}
		if hasPeeled && st.HasEncodedObject(peeled) != nil {
...
func (r *fetchWalker) fetchObject(st storage.Storer, hash plumbing.Hash, obj plumbing.EncodedObject) (err error) {
	if st.HasEncodedObject(hash) == nil {
		return nil
	}

	h := hash.String()
	url, err := url.JoinPath(r.url.String(), "objects", h[:2], h[2:])
	if err != nil {
		return err
	}
	req, err := http.NewRequestWithContext(r.ctx, http.MethodGet, url, nil)
...
func (r *fetchWalker) fetch(st storage.Storer) error {
...
LOOP:
	for len(r.queue) > 0 {
		...
		err := r.fetchObject(st, objHash, obj)
		...
		case plumbing.TreeObject:
			tree, err := object.DecodeTree(st, obj)
			if err != nil {
				return err
			}
	...
	for packPath := range packs {
		...
		if err := packfile.UpdateObjectStorage(st, f); err != nil {
			_ = f.Close()
			return err
		}
