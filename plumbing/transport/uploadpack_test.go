...
func (s *UploadPackSuite) TestAdvertisedReferencesEmpty(c *C) {
	r, err := s.Client.NewSession(UploadPackServiceName, s.EmptyEndpoint, s.EmptyAuth)
	c.Assert(err, IsNil)
	defer func() { c.Assert(r.Close(), IsNil) }()

	ar, err := r.DiscoverReferences(context.TODO(), false, nil)
	c.Assert(err, Equals, ErrEmptyRemoteRepository)
	c.Assert(ar, IsNil)
}

func (s *UploadPackSuite) TestAdvertisedReferencesNotExists(c *C) {
	r, err := s.Client.NewSession(UploadPackServiceName, s.NonExistentEndpoint, s.EmptyAuth)
	c.Assert(err, IsNil)
	defer func() { c.Assert(r.Close(), IsNil) }()
...
func (s *UploadPackSuite) TestCallAdvertisedReferenceTwice(c *C) {
	r, err := s.Client.NewSession(UploadPackServiceName, s.Endpoint, s.EmptyAuth)
	c.Assert(err, IsNil)
	defer func() { c.Assert(r.Close(), IsNil) }()
...
func (s *UploadPackSuite) TestUploadPackNoChanges(c *C) {
	...
	reader, err := r.Fetch(context.Background(), req)
	c.Assert(err, Equals, ErrEmptyUploadPackRequest)
	c.Assert(reader, IsNil)
}
