...
type ReceivePackSuite struct {
	...
	NonExistentEndpoint *Endpoint
	EmptyAuth           AuthMethod
	Client              Transport
}

func (s *ReceivePackSuite) TestAdvertisedReferencesEmpty(c *C) {
	r, err := s.Client.NewSession(ReceivePackServiceName, s.EmptyEndpoint, s.EmptyAuth)
	c.Assert(err, IsNil)
	defer func() { c.Assert(r.Close(), IsNil) }()

	ar, err := r.DiscoverReferences(context.TODO(), true, nil)
	c.Assert(err, IsNil)
	c.Assert(ar.Head, IsNil)
}

func (s *ReceivePackSuite) TestAdvertisedReferencesNotExists(c *C) {
	r, err := s.Client.NewSession(ReceivePackServiceName, s.NonExistentEndpoint, s.EmptyAuth)
	c.Assert(err, IsNil)
	ar, err := r.DiscoverReferences(context.TODO(), true, nil)
	...
	writer, err := r.Push(context.Background(), req)
	c.Assert(err, Equals, ErrRepositoryNotFound)
...
func (s *ReceivePackSuite) TestCallAdvertisedReferenceTwice(c *C) {
	r, err := s.Client.NewSession(ReceivePackServiceName, s.Endpoint, s.EmptyAuth)
	defer func() { c.Assert(r.Close(), IsNil) }()
	c.Assert(err, IsNil)
	ar1, err := r.DiscoverReferences(context.TODO(), true, nil)
	c.Assert(err, IsNil)
	c.Assert(ar1, NotNil)
	ar2, err := r.DiscoverReferences(context.TODO(), true, nil)
	c.Assert(err, IsNil)
...
func (s *ReceivePackSuite) TestDefaultBranch(c *C) {
	...
	info, err := r.DiscoverReferences(context.TODO(), true, nil)
	c.Assert(err, IsNil)
...
func (s *ReceivePackSuite) receivePackNoCheck(c *C, ep *Endpoint,
	...
	r, err := s.Client.NewSession(ReceivePackServiceName, ep, s.EmptyAuth)
	...
	return r.Push(context.Background(), req)
}

func (s *ReceivePackSuite) receivePack(c *C, ep *Endpoint,
	req *packp.ReferenceUpdateRequest, fixture *fixtures.Fixture,
...
func (s *ReceivePackSuite) checkRemoteHead(c *C, ep *Endpoint, head plumbing.Hash) {
	s.checkRemoteReference(c, ep, "refs/heads/master", head)
}

func (s *ReceivePackSuite) checkRemoteReference(c *C, ep *Endpoint,
	refName string, head plumbing.Hash) {

	r, err := s.Client.NewSession(UploadPackServiceName, ep, s.EmptyAuth)
	c.Assert(err, IsNil)
	defer func() { c.Assert(r.Close(), IsNil) }()
	ar, err := r.DiscoverReferences(context.TODO(), true, nil)
