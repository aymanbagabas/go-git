...
// Archivable is implemented by Sessions that support git-upload-archive.
// Stream transport sessions always implement this. HTTP sessions implement
// it when the server supports the upload-archive service.
...
// response and sideband-encoded archive stream from r. Returns an
// io.ReadCloser for the archive data (sideband channel 1).
//
// Wire protocol:
//
...
func Archive(ctx context.Context, w io.WriteCloser, r io.Reader, req *ArchiveRequest) (io.ReadCloser, error) {
	...
	demuxer := sideband.NewDemuxer(sideband.Sideband, rd)
	if req.Progress != nil {
		demuxer.Progress = req.Progress
	}

	return io.NopCloser(demuxer), nil
}
