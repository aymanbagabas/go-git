...
func readAllowUnreachable(st storage.Storer) (value, ok bool) {
	...
	s := cfg.Raw.Section("uploadArchive")
	v := s.Option("allowUnreachable")
	if v == "" {
		return false, false
	}
	return v == "true", true
...
func writeArchive(_ context.Context, st storage.Storer, mux *sideband.Muxer, args []string, allowUnreachable bool) error {
	...
	switch format {
	case "tar":
		return writeTarArchive(st, w, tree, prefix, paths, commitTime)
	case "tar.gz", "tgz":
		gw := gzip.NewWriter(w)
		...
		return gw.Close()
	case "zip":
		return writeZipArchive(st, w, tree, prefix, paths, commitTime)
	default:
...
func resolveTreeish(st storage.Storer, treeish string, allowUnreachable bool) (*object.Tree, time.Time, error) {
	// Handle ref:path syntax (e.g. v1.0:Documentation)
	var subPath string
	...
	if !allowUnreachable {
		// Reject raw SHA-1 hashes and anything that's not a simple ref name.
